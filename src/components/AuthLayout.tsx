...
export default function AuthLayout({ children, title }: { children: React.ReactNode; title: string }) {
  return (
    <div className="relative min-h-screen bg-gradient-to-b from-white via-slate-50 to-slate-100">
      ...
      <header className="max-w-6xl mx-auto px-4 py-4 flex items-center gap-2">
        <Link to="/" className="flex items-center gap-2">
          <Sparkles />
          <span className="font-extrabold tracking-tight">SportConnectIA</span>
        </Link>
      </header>

      <main className="max-w-6xl mx-auto px-4 pb-16 grid md:grid-cols-2 gap-10 items-center">
        {/* Ilustración simple */}
        <div className="order-2 md:order-1">
          <div className="aspect-[4/3] rounded-3xl bg-white/70 border border-white/60 shadow-xl grid place-items-center">
            <div className="text-slate-600 text-center px-6">
              ...
              <p className="text-sm mt-1">Programas, retos y seguimiento inteligente</p>
            </div>
          </div>
        </div>
