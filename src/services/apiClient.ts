...
export const SPORTS_BASE = isDev
  ? "/sports"
  : (import.meta.env.VITE_SPORTS_URL as string);
export const RECO_BASE = isDev
  ? "/reco"
  : (import.meta.env.VITE_RECO_URL as string);
export const CHAT_BASE = isDev
  ? "/chat"
...
async function request<T>(
  ...
  if (!res.ok) {
    ...
    // Adicionar informação de status ao erro
    if (typeof parsed === "object" && parsed !== null) {
      parsed.status = res.status;
      parsed.statusText = res.statusText;
    }

    console.error("❌ Erro na requisição:", parsed);
    throw parsed;
  }
